...
(
	...
	input  [KERNEL_SIZE*DATA_WIDTH-1:0]ifmap_data_i,
...
generate 
	for(k = 0; k < KERNEL_SIZE; k = k + 1) begin: multi_line_shift_ram
		...
		(
			...
			.ifmap_data_valid_i(ifmap_data_valid_i),
		
			.tap_data_valid_o(w_shift_line_data_valid[k]),
			.shift_data_in_i(ifmap_data_i[(k+1)*DATA_WIDTH-1-:DATA_WIDTH]),
			.tap_data_o(w_tap_data[k])
		);
	end
endgenerate

assign s_shift_ram_data_valid = &w_shift_line_data_valid[2];
...
always @(posedge clk or negedge rst_n) begin
	...
	else if(ifmap_data_valid_i) begin
		case(mode_sel_i)
			MODE_NORMAL_CONV: begin
				r_window_data_regs[0][0] <= w_tap_data[2][(0+1)*DATA_WIDTH-1-:DATA_WIDTH];
				r_window_data_regs[0][1] <= w_tap_data[2][(1+1)*DATA_WIDTH-1-:DATA_WIDTH];
				r_window_data_regs[0][2] <= w_tap_data[2][(2+1)*DATA_WIDTH-1-:DATA_WIDTH];
				...
				r_window_data_regs[2][0] <= w_tap_data[0][(0+1)*DATA_WIDTH-1-:DATA_WIDTH];
				r_window_data_regs[2][1] <= w_tap_data[0][(1+1)*DATA_WIDTH-1-:DATA_WIDTH];
				r_window_data_regs[2][2] <= w_tap_data[0][(2+1)*DATA_WIDTH-1-:DATA_WIDTH];
			end
			MODE_PW_CONV: begin
				for(i = 0; i < KERNEL_SIZE; i = i + 1) begin
					for(j = 0; j < KERNEL_SIZE; j = j + 1) begin
						r_window_data_regs[i][j] <= ifmap_data_i[(0+1)*DATA_WIDTH-1-:DATA_WIDTH];
					end
				end
			end
			default:;
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n) begin
		...
		window_data_valid_o <= (en && ifmap_data_valid_i) ? 1'b1 : 0;
	end
end
